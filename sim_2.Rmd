---
title: "Simulation: With time-dependent covariates"
...
# Simulate data
n <- 1000
x <- data.frame(trt = rbinom(n, 1, 0.5), x1 = rnorm(n), x2 = rnorm(n))
simdat <- simsurv(dist = "weibull", lambdas = 0.1, gammas = 1.5, betas = c(trt = -2, x1 = 1),
                  x = x, tde = c(trt = 5), tdefunction = "log", maxt = 7)
y <- simdat[, -1]
colnames(y)[1] <- "time"
dat <- cbind(y, x)
...
ggplot(dat_temp, aes(x = time)) + 
  geom_density(aes(colour = trt, fill = trt), alpha = 0.1) +
  theme_minimal() +
  scale_fill_viridis_d() +
  scale_colour_viridis_d() +
  labs(x = "Time", y = "Density", fill = "Treatment", colour = "Treatment",
       title = "Occurrence of the event over time by treatment group")
  ...
  callr::r(function(model, train, test) {
    ...
    if (model == "CoxTime") {
      model <- coxtime(Surv(time, status) ~ ., data = train, verbose = FALSE, epochs = 500L,
                     early_stopping = TRUE, frac = 0.33, batch_size = 512L, patience = 10L,
                     dropout = 0.1)
    } else if (model == "DeepHit") {
      model <- deephit(Surv(time, status) ~ ., data = train, verbose = FALSE, epochs = 500L,
                     early_stopping = TRUE, frac = 0.33, patience = 10L, cuts = 15L, 
                     batch_size = 512L, dropout = 0.1)
    } else if (model == "DeepSurv") {
      model <- deepsurv(Surv(time, status) ~ ., data = train, verbose = FALSE, epochs = 100L,
                      early_stopping = TRUE, frac = 0.33, patience = 10L, batch_size = 512L,
                      dropout = 0.1)
    } else {
...
grad_cox <- surv_grad(exp_coxtime, target = "survival", instance = c(1,4))
grad_deephit <- surv_grad(exp_deephit, target = "survival", instance = c(1,4))
grad_deepsurv <- surv_grad(exp_deepsurv, target = "survival", instance = c(1,4))
...
sg_cox <- surv_smoothgrad(exp_coxtime, target = "survival", instance = c(1,4), n = 50, noise_level = 0.4)
sg_deephit <- surv_smoothgrad(exp_deephit, target = "survival", instance = c(1,4), n = 50, noise_level = 0.4)
sg_deepsurv <- surv_smoothgrad(exp_deepsurv, target = "survival", instance = c(1,4), n = 50, noise_level = 0.4)
...
x_ref <- list(as.matrix(test[4, -c(1,2)]))
intgrad_cox <- surv_intgrad(exp_coxtime, target = "survival", x_ref = x_ref, n = 200)
intgrad_deephit <- surv_intgrad(exp_deephit, target = "survival", x_ref = x_ref, n = 200)
intgrad_deepsurv <- surv_intgrad(exp_deepsurv, target = "survival", x_ref = x_ref, n = 200)
...
inst <- 1
shap_cox <- surv_gradSHAP(exp_coxtime, target = "survival", instance = inst, n = 50, num_samples = 50)
shap_deephit <- surv_gradSHAP(exp_deephit, target = "survival", instance = inst, n = 50, num_samples = 50)
shap_deepsurv <- surv_gradSHAP(exp_deepsurv, target = "survival", instance = inst, n = 50, num_samples = 50)
